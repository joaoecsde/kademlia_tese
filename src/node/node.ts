...
import { hashKeyAndmapToKeyspace, XOR } from "../utils/nodeUtils";
import AbstractNode from "./abstractNode/abstractNode";
import { ALPHA, BIT_SIZE } from "./constants";
import { DHTOperations } from "./dhtOperations";
...
class KademliaNode extends AbstractNode {
	...
	constructor(id: number, port: number) {
		...
		// Initialize DHT Operations
		this.dhtOps = new DHTOperations(this);
	...
	// === DHT OPERATION DELEGATIONS ===
	// All DHT operations now delegate to the DHTOperations class

	public async store(key: number, value: string) {
		return this.dhtOps.store(key, value);
	}

	public async secureStore(key: number, value: string): Promise<any> {
		return this.dhtOps.secureStore(key, value);
	}

	public async findValue(value: string): Promise<FindValueResponse | null> {
		return this.dhtOps.findValue(value);
	}
	...
	public async registerAsGateway(
		...
		return this.dhtOps.registerAsGateway(blockchainId, endpoint, pubKey);
	}
	...
	public getCryptoStats(): {
		...
		return this.dhtOps.getCryptoStats();
	}

	private parseGatewayData(data: string): IGatewayInfo[] {
		return this.dhtOps.parseGatewayData(data);
	}

	// === MESSAGE HANDLING (kept in node.ts) ===
